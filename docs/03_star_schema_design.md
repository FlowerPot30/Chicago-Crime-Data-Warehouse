...
|`arrest_status_key`|bigint (FK)||Derived|`fact_crime.arrest_status_key` always points to the row in `dim_arrest_status` where `is_current=true` for that case. **1st**: Close the existing current row in `dim_arrest_status` - set `end_date` to the new `effective_date` and `is_current=false` **2nd**: Insert the new current row in `dim_arrest_status` with `is_current=true` **3rd**: Update `fact_crime.arrest_status_key` for that `case_number` to the newly inserted row's key|
